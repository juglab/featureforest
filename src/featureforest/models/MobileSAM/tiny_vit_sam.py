...
class TinyViTBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if H == self.window_size and W == self.window_size:
            ...
            # window partition
            x = x.view(B, nH, self.window_size, nW, self.window_size, C).transpose(
                2, 3).reshape(B * nH * nW, self.window_size * self.window_size, C)
            x = self.attn(x)
            # window reverse
...
class TinyViT(nn.Module):
    ...
    def forward_features(self, x):
        # x: (N, C, H, W)
        patch_embbedings = self.patch_embed(x)

        x = self.layers[0](patch_embbedings)
        start_i = 1
        ...
        x = x.permute(0, 3, 1, 2)
        x = self.neck(x)
        return x, patch_embbedings, interm_embeddings

    def forward(self, x):
        x, patch_embbedings, interm_embeddings = self.forward_features(x)
        # x = self.norm_head(x)
        # x = self.head(x)
